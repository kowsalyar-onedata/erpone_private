...
while True:
    url = (
        ...
        f"workItems(id,author(login,fullName),dateSpent,minutes,text)"
...
for issue in all_issues:
    ...
    if not work_items:
        work_items = [{"author": None, "dateSpent": None, "minutes": None, "text": None}]
    ...
    for wi in work_items:
        row = {
            ...
            # 🕒 Work item details
            "Work Author": wi.get("author", {}).get("fullName") if wi.get("author") else None,
            "Work Date": wi.get("dateSpent"),
            "Work Minutes": wi.get("minutes"),
            "Work Description": wi.get("text"),
        }
        rows.append(row)
...
# 🕒 Convert timestamps
for col in ["Created", "Updated", "Resolved", "Due Date", "Work Date"]:
    if col in df.columns:
        df[col] = pd.to_datetime(df[col], errors="coerce", unit="ms")

# 💾 Save results
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
csv_name = f"youtrack_erpo_with_workitems_{timestamp}.csv"
df.to_csv(csv_name, index=False, encoding="utf-8-sig")

print("\n✅ All ERPOne issue data (including Work Items) saved to:")
print(csv_name)
